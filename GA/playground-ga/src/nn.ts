...
export class Activations {
  ...
  public static SIN: ActivationFunction = {
    output: x => Math.sin(x),
    der: x => Math.cos(x)
  };
  public static RBF: ActivationFunction = {
    output: x => 1/ (1+x*x),
    der: x => {
	let output = Activations.RBF.output(x);
	return output * output * (- 2 * x);
     }
  };
}
...
export function countWeights(network: Node[][]) {
  let numowei = 0;
  for (let layerIdx = 1; layerIdx < network.length; layerIdx++) {
    let currentLayer = network[layerIdx];
    if (layerIdx < network.length - 1) numowei+=currentLayer.length; // biases
    for (let i = 0; i < currentLayer.length; i++) {
      let node = currentLayer[i];
       numowei+=node.inputLinks.length; // weights
    }
  }
  return numowei;
}

/* this function must be compatible with countWeights! */
export function setWeights(network: Node[][],genAlgX: number[],dim: number) {
  let numowei = 0;
  for (let layerIdx = 1; layerIdx < network.length; layerIdx++) {
    let currentLayer = network[layerIdx];
    for (let i = 0; i < currentLayer.length; i++) {
	...
	node.bias = genAlgX[numowei];
        numowei++;
        if (numowei>=dim) return numowei;
     }
      for (let j = 0; j < node.inputLinks.length; j++) {
        let link = node.inputLinks[j];
        if (link.isDead) {
...
/* this function can be used only with GA and should be compatible with countWeights! */
export function setLinkWeights(network: Node[][], genAlgX: number[], genAlgY: number[], dim: number) {
  let numowei = 0;
  for (let layerIdx = 1; layerIdx < network.length; layerIdx++) {
    let currentLayer = network[layerIdx];
    for (let i = 0; i < currentLayer.length; i++) {
      let node = currentLayer[i];
      if (layerIdx < network.length - 1) {
	node.bias = genAlgY[numowei]-0.5; // x values are not used  for the biasses
        numowei++;
        if (numowei>=dim) return numowei;
     }
      for (let j = 0; j < node.inputLinks.length; j++) {
        let link = node.inputLinks[j];
	if (genAlgX[numowei] < 0.5) {
		link.weight = 0;
             	link.isDead = true;
	}
	else {
		link.weight = genAlgY[numowei]-0.5; // initialised by additional GA components 
             	link.isDead = false;
        }
        numowei++;
        if (numowei>=dim) return;
      }
    }
  }
}
