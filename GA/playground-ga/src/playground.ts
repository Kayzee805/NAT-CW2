...
let population: genalg.Population = null;
let lossTrain = 0;
let lossTest = 0;
...
function updateUI(firstStep = false) {
  ...
  d3.select("#GENALG-number").text(humanReadable(GENALGout));
...
function oneGENALGStep(): number {
  iter++;
  GENALGout = population.updatePopulation(network,trainData,testData); /* changes the network to the fittest from the population */
  // Compute the loss.
  lossTrain = getLoss(network, trainData);
  lossTest = getLoss(network, testData);
  updateUI();
  return GENALGout; // this will go to the brower window.
}
...
function reset(onStartup=false) {
  ...
  lossTest = getLoss(network, testData);
  drawNetwork(network);
  population = genalg.buildPopulation(nn.countWeights(network));
  updateUI(true);
};
